...
public class PUTUpdateEmployee {
	...
	public void status_code_is_for_updateEmployee_API(int int1) {
		
		int statusCode = response.getStatusCode();
		response.then().assertThat().statusCode(int1);
		System.out.println("There is status code: " +statusCode);
	}

	@Then("user verifies employee name was updated")
	public void user_verifies_employee_name_was_updated() {
		response.then().body("employee[0].emp_firstname", equalTo("Yulia"));
		boolean checkPoint = response.asString().contains("Yulia");
		
		if (checkPoint) {
			System.out.println("Firstname employee is match");
		}
	}
